...
def generate_scenario(sequence: list[str, int, float], good_op: sl.Opinion, bad_op: sl.Opinion, rng) -> tuple[any, any]:
    ...
    def tag(ops_list: list[sl.Opinion], segments: list[dict[str, any]], label: str) -> list[sl.Opinion]:
        try:
    ...
    def generate_sl_opinions(target_op: sl.Opinion, variance: float, num_samples: int = 1) -> list[sl.Opinion]:
        ...
        return [
            sl.Opinion2d(sl.DirichletDistribution2d().from_evidences(np.maximum(evidence_target + n, 0.0)))
            for n in noise
        ]
    ...
    def transition(
        ...
        return [
            generate_sl_opinions(sl.Opinion(b, d), variance=variance, num_samples=1)[0]
    ...
    for label, length, variance in sequence:
        if label == "good":
            ...
            ops.extend(
                tag(
                    transition(current_op=current, target_op=target, variance=variance, num_samples=length),
                    segments,
                    label="transition",
                )
            )
