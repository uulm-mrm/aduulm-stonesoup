...
def post_hoc_conflict_detection(memory) -> None:
    ...
    def monte_carlo_ll(long_op: sl.Opinion, short_op: sl.Opinion) -> None:
        ...
        # count more extreme outcomes (cdf)
        tail_events = np.sum(mc_log_likelihoods <= log_likelihood)
        pval = tail_events / num_simulations
        pvals.append(pval)

    def kld_dirichlet(long_op: sl.Opinion, short_op: sl.Opinion) -> None:
        def kl_div_dirichlet(alpha: np.array, beta: np.array) -> float:
            alpha_0 = np.sum(alpha)
            beta_0 = np.sum(beta)
            # implement numerically more stable version compared to Obsidian
            diff_0 = gammaln(alpha_0) - gammaln(beta_0)
            sum_1 = np.sum(gammaln(beta) - gammaln(alpha))
            sum_2 = np.sum((alpha - beta) * (digamma(alpha) - digamma(alpha_0)))
            return diff_0 + sum_1 + sum_2

        dirichlet_lt, dirichlet_st = long_op.as_dirichlet(), short_op.as_dirichlet()
        alphas_lt, alphas_st = np.array(dirichlet_lt.evidences + 1), np.array(dirichlet_st.evidences + 1)

        kl_divs.append(kl_div_dirichlet(alphas_lt, alphas_st))

    long_size = memory.get_long_size()
    if long_size == 0 and not memory.is_last_conflicted():
        pvals.append(math.nan)
        kl_divs.append(math.nan)
        dcs.append(math.nan)
        return
    ...
    kld_dirichlet(long_op, short_op)
...
def plot(segments: list[dict[str, any]]) -> None:
    ...
    if len(pvals) > 0:
        axes[2].plot(pvals, label="pval (MC)", color="tab:blue")
        axes[2].hlines(0.05, 0, len(pvals), linestyles="--", color="tab:blue")
        axes[2].fill_between(
            np.arange(len(pvals)),
            0,
            1,
    ...
    if len(kl_divs) > 0:
        ax_kl = axes[2].twinx()
        ax_kl.plot(kl_divs, label="KL-Div", color="tab:green")
        ax_kl.vlines(np.argwhere(resets), ymin=0.0, ymax=np.max(kl_divs), linestyles=":", color="tab:red")
        lines_kl, labels_kl = ax_kl.get_legend_handles_labels()
        lines_dc += lines_kl
        labels_dc += labels_kl
...
def write_csv(base_path="/tmp") -> None:
    ...
    headers = ["inp-pp", "inp-u", "buffer-pp", "buffer-u", "resets", "dc", "pval", "kld"]
    np_arrs_to_write = [inp, buffer, resets.astype(int), dcs, pvals, kl_divs]
    cols = []
...
ops, segments = generate_scenario(scenarios["single_jump"], GOOD_OP, BAD_OP, rng)
