...
SWEEP_EXPORT_ONLY = os.getenv('SA_SWEEP_EXPORT_ONLY', '0') == '1'
if SWEEP_EXPORT_ONLY:
    matplotlib.use('Agg')
else:
...
SIMULATION_SEED = int(os.getenv('SA_SIMULATION_SEED', '0'))
MEASUREMENT_SEED = int(os.getenv('SA_MEASUREMENT_SEED', str(SIMULATION_SEED + 1)))
...
# Ground-truth labels used by the parameter-sweep evaluation.
# Intervals are half-open: [start, end).
EVALUATION_EVENTS = [
    {'name': 'measurement_outliers', 'start': 100, 'end': 200},
    {'name': 'measurement_noise_x_increased', 'start': 300, 'end': 400},
    {'name': 'measurement_noise_xy_decreased', 'start': 500, 'end': 600},
    {'name': 'truncated_gaussian_measurement_noise', 'start': 700, 'end': 800},
    {'name': 'coordinated_turn_model_mismatch', 'start': 900, 'end': 1000},
    {'name': 'process_noise_x_increased', 'start': 1100, 'end': 1300},
]
...
if SWEEP_EXPORT_ONLY:
    # These tables are needed only by the later plotting/comparison section,
    # not for exporting the raw PIT histories.
    entropy_thresholds = {}
    opinion_thresholds = {}
    adjusted_opinion_thresholds = {}
else:
...
THRESHOLD = (
    1.0
    if SWEEP_EXPORT_ONLY
    else opinion_thresholds[f"{W}, {calculate_lt_evidence(W, DISCOUNT)}, 0.01"]
)  # Threshold is irrelevant while exporting raw PIT data.
...
# ------------------------------------------------------------------
# Optional raw-data export for an external parameter sweep
# ------------------------------------------------------------------
def export_parameter_sweep_cache(
    output_path,
    radial_pit,
    ...
    """
    Stores only quantities that are independent of the SL mapping and LTST
    parameters. A parameter sweep can therefore reuse one KF realization and
    rebuild the SL opinions for many parameter combinations without rerunning
    the Kalman filter.
    """
    output_path = Path(output_path)
    output_path.parent.mkdir(parents=True, exist_ok=True)

    radial_pit = np.asarray(radial_pit, dtype=float)
    whitened_innovations = np.asarray(whitened_innovations, dtype=float)

    if whitened_innovations.ndim != 2 or whitened_innovations.shape[1] < 2:
        raise ValueError(
            "whitened_innovations must have shape (num_steps, >=2)."
        )

    component_x_pit = norm.cdf(whitened_innovations[:, 0])
    component_y_pit = norm.cdf(whitened_innovations[:, 1])

    n_samples = len(radial_pit)
    if not (
        len(component_x_pit) == n_samples
        and len(component_y_pit) == n_samples
    ):
        raise ValueError("The exported PIT histories must have equal length.")

    fault_mask = np.zeros(n_samples, dtype=bool)
    event_id = np.full(n_samples, -1, dtype=int)

    for idx, event in enumerate(events):
        start = max(0, int(event["start"]))
        end = min(n_samples, int(event["end"]))
        fault_mask[start:end] = True
        event_id[start:end] = idx

    # Ground-truth-based position error is exported only for offline analysis;
    # it is not used by the runtime self-assessment.
    truth_by_time = {state.timestamp: state for state in truth}
    position_error = np.full(n_samples, np.nan, dtype=float)
    for idx, estimate in enumerate(track):
        if idx >= n_samples:
            break
        ...
        error_xy = x_est[[0, 2]] - x_gt[[0, 2]]
        position_error[idx] = float(np.linalg.norm(error_xy))

    metadata = {
        "simulation_seed": int(simulation_seed),
        "measurement_seed": int(measurement_seed),
        "dt_seconds": float(dt_seconds),
        "activate_disturbances": bool(activate_disturbances),
        "events": events,
        "num_samples": int(n_samples),
        "description": (
            "Raw KF/PIT realization for SL mapping and LTST parameter sweeps. "
            "Intervals are half-open [start, end)."
        ),
    }
    ...
    np.savez_compressed(
        output_path,
        radial_pit=radial_pit,
        component_x_pit=component_x_pit,
        component_y_pit=component_y_pit,
    ...
    print(f"Parameter-sweep cache written to: {output_path}")


SWEEP_CACHE_PATH = os.getenv("SA_SWEEP_CACHE_PATH")
if SWEEP_CACHE_PATH:
    export_parameter_sweep_cache(
        output_path=SWEEP_CACHE_PATH,
        radial_pit=u_history,
        whitened_innovations=nu_white_full_history,
        track=track,
        truth=truth,
