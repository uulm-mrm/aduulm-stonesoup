...
hypothesiser = {sensor_name: DistanceHypothesiser(predictor, updaters[sensor_name], measure=Mahalanobis(),
                                                  missed_distance=sigma_cutoff_gating) for sensor_name in sensors}
# %%
...
data_associators = {sensor_name: NearestNeighbour(hypothesiser[sensor_name]) for sensor_name in sensors}

# %%
# Run the multi-sensor Kalman filter with the associator
# ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
for i in range(num_steps-1):
    ...
    for n, sensor_name in enumerate(sensors):
        hypotheses = data_associators[sensor_name].associate({state},
                                                             measurements[n],
                                                             start_time + timedelta(seconds=i))
        hypothesis = hypotheses[state]
        ...
        # multi-sensor self-assessments
        num_meas = len(measurements[n])
        multisensor_selfassessors.assess(sensor_name, z_p, S_p, nn_associated_meas, num_meas)
        ...
        # nis assessor
        nis_measures = nis_assessors[sensor_name].assess(z_p, S_p, nn_associated_meas)
...
for sensor_name in sensors:
    plot_selfassessment_with_nis(selfassessors_measures_combined_histories[sensor_name],
                                 ...
                                 assessor_type='NNSelfAssessor')

plot_multisensor_selfassessment(multisensor_measures_combined_history,
                                assessor_type='Multi-Sensor NNSelfAssessor: Combined Opinion')
